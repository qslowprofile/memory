...
from mk_arch_core import (
    BUCKET_FILES,
    _llm_extract_entities_and_relations,
...
def recompute_graph_fields(rows: Sequence[Dict[str, Any]]) -> Tuple[List[Dict[str, Any]], Dict[str, int]]:
    ...
    for row in rows:
        ...
        if text:
            # 优先尝试 LLM 联合抽取实体+关系，失败时 fallback 到规则
            llm_er = _llm_extract_entities_and_relations(text, bucket=bucket)
            if llm_er is not None:
                new_entities, new_relations_tuples = llm_er
                # 用规则抽取补充 LLM 可能遗漏的实体
                for re_ent in extract_entities(text):
                    if re_ent not in new_entities and len(new_entities) < 16:
                        new_entities.append(re_ent)
            else:
...
def apply_repairs(
    ...
    if reports_dir and reports_dir.is_dir():
        try:
            ...
            for old_report in report_files[_MAX_REPORTS:]:
                old_report.unlink(missing_ok=True)
            if len(report_files) > _MAX_REPORTS:
                actions.append(
                    {
                        "action": "cleanup_old_reports",
                        "removed": len(report_files) - _MAX_REPORTS,
                        "kept": _MAX_REPORTS,
                    }
                )
        except OSError:
